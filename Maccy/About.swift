...
class About {
  ...
  private var links: NSMutableAttributedString {
    let string = NSMutableAttributedString(string: "Website│GitHub│Support",
                                           attributes: [NSAttributedString.Key.foregroundColor: NSColor.labelColor])
    string.addAttribute(.link, value: "https://github.com/cleancoder8/copy-cat", range: NSRange(location: 0, length: 7))
    string.addAttribute(.link, value: "https://github.com/cleancoder8/copy-cat", range: NSRange(location: 8, length: 6))
    string.addAttribute(.link, value: "https://github.com/cleancoder8/copy-cat/issues", range: NSRange(location: 15, length: 7))
    return string
  }
  ...
  private var credits: NSMutableAttributedString {
    let credits = NSMutableAttributedString(string: "",
                                            attributes: [NSAttributedString.Key.foregroundColor: NSColor.labelColor])
    credits.append(NSAttributedString(string: "Copy Cat — based on Maccy 2.7.1\nCopyright © Alexey Rodionov · MIT License\n\n"))
